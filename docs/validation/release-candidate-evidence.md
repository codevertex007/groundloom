...
|---|---|---|
| Backend unit/contract/security | `python -m pytest -q` | 30 passed; one optional-provider test skips when extras are absent |
...
| Frontend API-client tests | `cd frontend; npm test` | 2 passed: typed retryable errors/correlation and SSE reconnect cursor/offline behavior |
...
| Migration | `python backend/scripts/migrate.py`, `python backend/scripts/verify_migrations.py`, and migration status query | `001_initial_domain_schema`, `002_ingestion_jobs_and_provider_adapters`, `003_retention_deletion_and_export_leases` |
