...
| Derived-index/retrieval/delegated/outbox workers | `backend/tests/test_index_rebuild_worker.py`, `backend/tests/test_retrieval_adapters.py`, `backend/tests/test_delegation_recovery.py`, `backend/tests/test_outbox_delivery.py` | Scoped lexical/embedding index rebuild, deterministic/provider retrieval boundaries, failed delegated-task retry, and at-least-once outbox replay are covered by bounded workers |
