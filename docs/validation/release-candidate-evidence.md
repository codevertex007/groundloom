...
|---|---|---|
| Backend unit/contract/security | `python -m pytest backend/tests -q -rA` | 78 passed; two optional-provider tests and four opt-in deployment-integration tests skip in the default local environment; the provider probe passes both optional tests and the disposable Docker run passes all four deployment tests |
...
| Python types | `python -m mypy backend/app backend/scripts` | Passed; 37 source files |
...
| Retrieval envelope | `python backend/scripts/benchmark_local.py --requests 20` | SQLite/filesystem adapter: p50 9.494 ms, p95 15.423 ms, max 15.423 ms; not a production SLO |
