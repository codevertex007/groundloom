...
The local adapter gate is the evidence boundary for development: 37 backend tests (plus one optional-provider skip when extras are absent), Ruff, mypy, documentation validation, deterministic evaluation telemetry, signed-identity tests, checksum-backed backup/restore tests, explicit derived-index rebuild, a synthetic retrieval benchmark, delegation retry/reconciliation and trajectory tests, leased agent-worker dispatch, worker health/heartbeat and budget-stop tests, 3 Playwright E2E/accessibility tests, and the frontend production build pass; the browser journey covers project creation, source-grounded drafting, proposal review, acceptance, settings persistence, command-palette navigation, and serious/critical axe violations. The unchecked production gates above remain intentionally open because this environment has no live Deep Agents provider, Postgres/pgvector instance, external object storage, identity provider, Langfuse deployment, production restore rehearsal, load report, or release-owner approval.
