...
| `FR-AGENT-001..002` | `backend/tests/test_api_vertical_slice.py::test_project_source_grounded_run_and_replay`, `::test_plan_approval_interrupt_resumes_same_run_and_thread`, `backend/tests/test_adapters_and_jobs.py::test_agent_worker_claims_queued_runs_and_preserves_inline_local_mode`; durable adaptive local trajectory, approval interrupt, same-thread resume, redacted usage/budget metadata, and leased worker dispatch |
