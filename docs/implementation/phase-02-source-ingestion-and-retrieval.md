...
- [x] `IMPL-02-004` Versioned deterministic chunking/indexing through the pinned LangChain recursive splitter and one ingestion/rebuild implementation, with bounded embedding batches through deterministic local or LangChain `OpenAIEmbeddings` providers and an explicit scoped, idempotent rebuild worker that mirrors deployment vectors into the scoped pgvector derived table through migration 015.
- [x] `IMPL-02-005` Authorized bounded hybrid lexical/semantic evidence search with selected-version filtering, block-level aggregation across multiple chunk vectors, bounded PostgreSQL semantic/lexical candidate loading, deterministic or LangChain `CohereRerank` reranking, same-version neighbor expansion, duplicate suppression, and typed provider failure handling.
- [x] `IMPL-02-006` Passage read/navigation APIs and citation-lineage primitives.
- [x] `IMPL-02-007` Prompt-injection/sanitization signals, deterministic source quarantine, bounded HTTP scanner adapter, and adversarial source fixture coverage.
