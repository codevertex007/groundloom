...
- [x] `IMPL-07-010` Reference-aligned studio shell and narrow viewport hardening; the sidebar/brand hierarchy now follows the supplied dark studio reference, Groundloom and Knowledge Studio remain a compact stacked wordmark, and mobile layouts collapse the rail and wrap controls without horizontal overflow. The static UI contract suite protects the branding and responsive CSS invariants.
