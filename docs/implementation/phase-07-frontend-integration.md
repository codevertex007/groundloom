...
- [x] `IMPL-07-010` Reference-aligned studio shell and narrow viewport hardening; the sidebar/brand hierarchy now follows the supplied dark studio reference, Groundloom and Knowledge Studio remain a compact stacked wordmark, Projects/Sources use the reference inline header and centered content frame, project filters use the reference pill row, empty states use the reference dashed panel, and project cards expose the reference type/status/progress metadata. Mobile layouts collapse the rail and wrap controls without horizontal overflow. The static UI contract suite protects the branding and responsive CSS invariants.
