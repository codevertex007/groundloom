...
| 06 Quality, evaluation, and export | completed-local | Deterministic validators, versioned rubric/baseline runner, redacted evaluation observation adapter, validation findings, idempotent PDF/DOCX/Markdown/HTML export, and preview/download are implemented; model semantic evaluator integration remains external-provider work. |
...
| 2026-08-25 | Security/reliability hardening | `python -m pytest -q` passed (26 tests); signed identity rejects missing/tampered bearer context and unsafe production configuration; MIME-spoofed DOCX failure persists; local backup manifest verifies; `benchmark_local.py --requests 20` reported a synthetic p95; parser, database, and benchmark cleanup paths were exercised. |
