...
| 2026-08-25 | Current local release audit | Backend 48 tests passed with two optional-provider skips in the default environment; the isolated pinned-provider probe passed both optional tests. Frontend native 5, component 3, and Playwright 5 tests passed, including source upload/evidence selection/citation navigation and canonical rejection safety; build, Ruff, mypy, migration verification, docs validation, compose validation, and npm audits passed. Runtime workers now use the dedicated worker DB URL, production API startup skips schema bootstrap, migrations initialize checkpoint schema through the migrator role, staging/production role separation is validated, and the local role-boundary regression tests pass. Retrieval benchmark reported p50 6.641 ms/p95 9.791 ms over 20 requests. `pip check` remains red only for the unrelated global Streamlit/protobuf conflict. |
