...
| 03 Primary agent foundation | completed-local | One persistent project thread, adaptive deterministic local runtime, local direct/delegated trajectory tests, durable todos/events, checkpoint seam, SSE replay, cancellation, pinned optional Deep Agents/Postgres adapter, provider safety boundary, approval interrupts, and redacted run usage/budget metadata are implemented; live provider/recovery evidence is deployment work. |
