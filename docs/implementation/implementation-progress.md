...
| 2026-08-25 | Current local release audit | Backend 41 tests passed with two optional-provider skips in the default environment; the isolated pinned-provider probe passed both optional tests. Frontend native 5, component 3, and Playwright 4 tests passed, including source upload/evidence selection/citation navigation; build, Ruff, mypy, migration verification, docs validation, and npm audits passed. Retrieval benchmark reported p50 9.544 ms/p95 46.369 ms over 20 requests. `pip check` remains red only for the unrelated global Streamlit/protobuf conflict. |
