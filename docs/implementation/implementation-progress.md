...
| 03 Primary agent foundation | completed-local | One persistent project thread, adaptive deterministic local runtime, local direct/delegated trajectory tests, durable todos/events, checkpoint seam, SSE replay, cancellation, pinned optional Deep Agents/Postgres adapter, fake-model graph compile, and provider safety boundary are implemented; live provider/recovery evidence is deployment work. |
| 04 Skills, memory, and subagents | completed-local | Versioned skill validation/publication, draft-only skill-author endpoint, scoped redacted memory, bounded delegated-task records with retry/reconciliation, and proposal-only tool registry are implemented and tested. |
...
| 07 Frontend integration | completed-local | Reference-informed React/Vite UI uses a shared API client, durable SSE replay/reconnect with connection state, native API-client tests, and real API contracts; browser smoke completed create → generate → review → accept on 2026-08-24. Automated component/a11y/visual/full-e2e evidence remains release work. |
...
| 2026-08-25 | Isolated provider adapter verification | Optional extras installed cleanly in a disposable venv; fake-model Deep Agents graph compile and optional provider contract test passed; Langfuse adapter outage probe remained bounded and did not touch product state; live Postgres/S3/provider credentials remain unavailable. |
| 2026-08-25 | Export, retention, and derived-index worker slice | Added migrations 003-004, durable export leases/worker, project deletion request/status API, legal-hold policy seam, scoped unshared source/object/checkpoint cleanup, explicit derived lexical-index rebuild API/worker, queued-export polling, audit/outbox completion, and four focused worker tests; full backend suite now 31 passed with one optional-provider skip, frontend API tests 3 passed, frontend build passed, and docs validation passed for 166 documents. |
