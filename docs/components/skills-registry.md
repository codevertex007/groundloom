...
Commands: create draft, fork, repair/update draft, validate, request approval, publish, deprecate, select version. Repair creates the next immutable draft version and records its predecessor; publication never mutates bytes. Resolver returns only authorized active metadata/packages and pins project/run projections.
