...
Owns rubric/evaluator versions, validation executions, findings, review items/overrides, evaluation datasets/results, and feedback references. Deterministic validators and semantic graders share a structured finding model but remain distinguishable. The deterministic local grader is mandatory for known invariants; an optional provider-versioned evaluator composes LangChain `ChatPromptTemplate`, `ChatOpenAI`, and Pydantic structured output over bounded inputs.
