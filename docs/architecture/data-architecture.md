...
Use database constraints for uniqueness, foreign keys, valid simple state invariants, idempotency, and optimistic version numbers. Application checks do not replace constraints. Prefer row-level security as defense in depth while retaining service authorization.
