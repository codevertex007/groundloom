...
- On PostgreSQL, force row-level policies over workspace-owned tables using a transaction-local trusted workspace setting; leased workers use a distinct `groundloom_worker` database role for bounded queue claims, while the service context marker remains metadata only.
